...
void keyboardHandler(struct InterruptRegisters* regs){
    char scanCode = inPortB(0x60) & 0x7f;   //vilken tangent?
    char press = inPortB(0x60) & 0x80;      //tryckt ner (0) eller upp (1) (släppt)
    switch(scanCode){
        case 42: //shift
            shiftCaps = !press;
            break;
        case 58: //capslock
            if(press == 0){
                capsLock = !capsLock;
                printf("caps key pressed!"); newl();
            }break;
        default:
            if(press == 0){
                char c[2];
                c[1] = 0;
                if(capsLock || shiftCaps){
                    c[0] = uppercase[scanCode];
                    printf(c);
                }else{
                    c[0] = lowercase[scanCode];
                    printf(c);
                }
            }
    }
